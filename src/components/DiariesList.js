...
const DiariesList = ({ onPlantSelect, userRole, searchQuery, diaryFilters }) => {
    ...
    // ĐIỂM SỬA 1: Có hàm `setAllFarmerDiaries` để cập nhật state
    ...
    useEffect(() => {
        ...
        // ĐIỂM SỬA 2: Thêm hàm để tải tất cả nhật ký về cho bộ lọc
        const getAllDiariesForFilter = async () => {
            if (user && userRole === 'farmer') {
                try {
                    const diariesRef = collection(db, 'diaries');
                    const q = query(diariesRef, where("userId", "==", user.uid));
                    const querySnapshot = await getDocs(q);
                    const diariesData = querySnapshot.docs.map(doc => ({...doc.data(), id: doc.id}));
                    setAllFarmerDiaries(diariesData); // Cập nhật danh sách nhật ký
                } catch (error) {
                    console.error("Lỗi khi lấy dữ liệu nhật ký để lọc:", error);
                }
            }
        };

        getPlants();
        getAllDiariesForFilter(); // Gọi hàm tải nhật ký
    ...
    const filteredPlants = useMemo(() => {
        ...
        if (userRole === 'farmer' && diaryFilters) {
            // Giờ allFarmerDiaries đã có dữ liệu để lọc
